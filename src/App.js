    ...
    <div>
      ...
      <div style={{padding:16,borderRadius:12,border:"0.5px solid rgba(128,128,128,0.2)",marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            ["totalLogements",isRes?"Nb logements résidence":"Nb total logements","number","Ex: 120"],
            ["nbLocataires75","Nb locataires 75+","number","Ex: 18"],
            ["pctSeniors75","% occupés 75+","number","Ex: 15"],
            ["tauxRotation","Taux rotation (%)","number","Ex: 5.2"],
            ["responsablePilotage","Responsable pilotage","text","Nom + fonction"],
            ...
            <div key={f[0]}>
              ...
              <input type={f[2]} value={form[f[0]]||""} onChange={function(e){setF(f[0],e.target.value);}} placeholder={f[3]}
                style={{width:"100%",fontSize:13,padding:"7px 10px",border:"0.5px solid rgba(128,128,128,0.3)",borderRadius:8,background:"transparent",color:"inherit",boxSizing:"border-box"}}/>
            </div>
          );})}
        </div>
        ...
        <div style={{marginTop:14}}>
          ...
          <AddressField
            value={form.adresse}
            onSelect={function(addr){setF("adresse",addr);}}
            placeholder={isRes?"12 rue des Lilas, Lyon…":"15 avenue de la République, Paris…"}
          />
          {form.adresse && (
            <div style={{marginTop:8,display:"flex",gap:8,flexWrap:"wrap"}}>
              {[
                {label:"Commune", val:form.adresse.city},
                {label:"Code INSEE", val:form.adresse.citycode},
                {label:"Département", val:form.adresse.departement},
                {label:"Région", val:form.adresse.region},
      ...
      <div style={{padding:16,borderRadius:12,border:"0.5px solid rgba(128,128,128,0.2)",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          ...
          <div style={{fontSize:11,color:"#888"}}>{saisieCount}/{totalKpis} renseignés</div>
        </div>
        ...
        <div style={{fontSize:10,color:"#888",textTransform:"uppercase",letterSpacing:"0.04em",display:"grid",gridTemplateColumns:"2fr 0.8fr 0.4fr 0.6fr 1fr",gap:6,padding:"0 10px 6px",borderBottom:"0.5px solid rgba(128,128,128,0.1)"}}>
          <span>Indicateur</span><span>Formule</span><span>Unité</span><span>Valeur</span><span>Benchmark national</span>
        </div>
        {t.kpis.map(function(kpi){
          ...
          return(
            <div key={kpi.id} style={{display:"grid",gridTemplateColumns:"2fr 0.8fr 0.4fr 0.6fr 1fr",gap:6,padding:"8px 10px",borderBottom:"0.5px solid rgba(128,128,128,0.06)",alignItems:"center"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:2}}>
                ...
                {b && <div style={{fontSize:9,color:"#aaa"}}>{b.source}</div>}
